        ...
        switch (element_data.type) {
            ...
            case 'blog_post':
                ...
                $(element_data.images).each(function(index, image_src) {
                    ...
                    if (SELF._options.lightbox) {
                        html += '<div class="img_overlay" id="' + element_data.photoId + '"><span class="magnifier" data-total="' + element_data.images.length + '" data-order="' + index + '" data-img="' + image_src + '"></span></div>';
                    }
                    html += '</div>';
                });
    ...
    */
    ...
    this._handleClick = function(e) {
        ...
        if (element.hasClass('timeline_overlay')) {
            SELF._closeLightBox();
        } else if (element.hasClass('img_overlay')) {
            SELF._openDiaglog(element, element.data('id'));
        } else if (element.hasClass('magnifier')) {
            //SELF._openLightBox(element, element.data('img'));
            SELF._openDialogMagnifier($(element).parent().attr('id'));
    ...
    // Add new things.
    this._openDiaglog = function(element) {
        showPicture(element.context['id']);
    };

    this._openDialogMagnifier = function(id) {
        showPicture(id);
    };
